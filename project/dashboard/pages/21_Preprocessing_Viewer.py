...
DARK_BG = '#0b0f19'
st.set_page_config(page_title="21 - Preprocessing Viewer", layout="wide", page_icon="🧠")
st.markdown(f"""
    <style>
...
st.title("🧽 21 - Preprocessing Viewer")
...
vol_orig = st.session_state['vol_original']
vol_pre = st.session_state['vol_preprocessed']
z = st.slider("Z-Slice", 0, vol_orig.shape[0]-1, vol_orig.shape[0]//2)
col1, col2 = st.columns(2)
with col1:
    st.subheader("Original")
    fig = go.Figure(data=go.Heatmap(z=vol_orig[z], colorscale='gray', showscale=False))
...
with col2:
    st.subheader("Stage 2 Preprocessed (Denoised & Corrected)")
    fig2 = go.Figure(data=go.Heatmap(z=vol_pre[z], colorscale='gray', showscale=False))
